...
- Не используйте актор в качестве `ObservableObject` в SwiftUI. У компилятора нет проблем, позволяющих вам это сделать, но `ObservableObject` будет ожидать, что он всегда будет работать в `main` потоке. Наличие актора с дорогостоящими вызовами приведет к лагам в вашем приложении.
