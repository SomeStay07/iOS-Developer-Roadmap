...
Поскольку не каждый человек может иметь домашнее животное, и тот человек, у которого может - быть один петомец, означается что поле `pet` для класса `Person` будет опциональным.
